...
export interface RenderCallbackParams<T extends object = Recordable> {
	schema: ComputedRef<FormSchema<T>>
	/** 响应式的表单数据对象 */
...
type ComponentSchema<T extends object = Recordable> =
	| {
			[K in ComponentType]: {
				...
				component: K
				/** 表单组件属性 */
				componentProps?: ComponentProps<K> | ((opt: RenderCallbackParams<T>) => ComponentProps<K>)
			}
	  }[ComponentType]
	| {
...
export type FormSchema<T extends object = Recordable> = ComponentSchema<T> & {
	...
	slot?: string
	/** 表单组件前置插槽 */
	beforeSlot?: string | ((renderCallbackParams: RenderCallbackParams<T>) => VNode | VNode[])
	/** 表单组件后置插槽 */
	afterSlot?: string | ((renderCallbackParams: RenderCallbackParams<T>) => VNode | VNode[])
