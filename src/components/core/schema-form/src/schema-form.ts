...
import type { ComponentInternalInstance, CSSProperties, EmitFn, EmitsToProps, ExtractPublicPropTypes } from 'vue'
import type { ColEx } from './types/component'
import type { ButtonProps, FieldMapToTime, FormSchema, RowProps } from './types/form'
...
export const schemaFormProps = {
	...
	/** 转化时间 */
	transformDateFunc: {
		type: Function as PropType<(date: unknown) => unknown>,
...
export const schemaFormEmits = {
	...
	reset: (formModel: Recordable<unknown>) => isObject(formModel),
	submit: (formModel: Recordable<unknown>) => isObject(formModel),
	advancedChange: () => true
}
...
export type SchemaFormProps<T extends object = Record<string, unknown>> = Omit<
